...
var modelWidth = 224.0;
var modelHeight = 224.0;
var modelChannel = 3;
var inferenceStart;
var inferenceEnd;
var classNames = {
...
var Classifier = /** @class */ (function () {
    function Classifier(displaySize) {
        // the predicted class
        this.mPredClass = "None";
        this.speedTime = 0.0;
    ...
    Classifier.prototype.load = function () {
        ...
        return new Promise(function (resolve, reject) {
            var modelPath = wx.env.USER_DATA_PATH + "/mobilenetv2_qat.onnx";
            // 判断之前是否已经下载过onnx模型
            wx.getFileSystemManager().access({
                path: modelPath,
                success: function (res) {
                    console.log("file already exist at: " + modelPath);
                    _this.createInferenceSession(modelPath).then(function () {
                        resolve();
                    });
                },
                fail: function (res) {
                    ...
                    var cloudPath = 'cloud://containertest-0gmw3ulnd8d9bc7b.636f-containertest-0gmw3ulnd8d9bc7b-1258211818/mobilenetv2_qat.onnx';
                    _this.downloadFile(cloudPath, function (r) {
                        ...
                        wx.getFileSystemManager().saveFile({
                            ...
                            success: function (res) {
                                console.log(res);
                                var modelPath = res.savedFilePath;
    ...
    Classifier.prototype.createInferenceSession = function (modelPath) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.session = wx.createInferenceSession({
                model: modelPath,
            ...
            // 监听error事件
            _this.session.onError(function (error) {
                console.error(error);
                reject(error);
            });
            _this.session.onLoad(function () {
                _this.ready = true;
                resolve();
            });
        });
    };
    Classifier.prototype.downloadFile = function (fileID, onCall) {
        if (onCall === void 0) { onCall = function () { }; }
        return new Promise(function (resolve, reject) {
            var task = wx.cloud.downloadFile({
                fileID: fileID,
    ...
    Classifier.prototype.isReady = function () {
        return this.ready;
    };
    Classifier.prototype.predClass = function () {
        return this.mPredClass;
    };
    // input is rgba uint8 data
    Classifier.prototype.preProcess = function (frame, dstInput) {
        return new Promise(function (resolve, reject) {
            var origData = new Uint8Array(frame.data);
            var hRatio = frame.height / modelHeight;
            var wRatio = frame.width / modelWidth;
            // resize data to model input size, uint8 data to float32 data,
            // and transpose from nhwc to nchw
            var origHStride = frame.width * 4;
            var origWStride = 4;
            var mean = [0.485, 0.456, 0.406];
            var reverse_div = [4.367, 4.464, 4.444]; // reverse of std = [0.229, 0.224, 0.225]
            var ratio = 1 / 255.0;
            var normalized_div = [ratio * reverse_div[0], ratio * reverse_div[1], ratio * reverse_div[2]];
            var normalized_mean = [mean[0] * reverse_div[0], mean[1] * reverse_div[1], mean[2] * reverse_div[2]];
            var idx = 0;
            for (var c = 0; c < modelChannel; ++c) {
                for (var h = 0; h < modelHeight; ++h) {
                    var origH = Math.round(h * hRatio);
                    var origHOffset = origH * origHStride;
                    for (var w = 0; w < modelWidth; ++w) {
                        var origW = Math.round(w * wRatio);
                        var origIndex = origHOffset + origW * origWStride + c;
    ...
    Classifier.prototype.detect = function (frame) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            return __generator(this, function (_a) {
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        _this.preProcess(frame, _this.modelInput).then(function () {
                            var xinput = {
                                shape: [1, 3, 224, 224],
                                data: _this.modelInput.buffer,
                                type: 'float32'
                            };
                            ...
                            _this.session.run({
                                ...
                                _this.getClass(index);
                                resolve();
                            })["catch"](function (error) { return console.log("runing error!: ", error.message); });
                        });
                    })];
            });
        });
    };
    Classifier.prototype.getClass = function (index) {
        var cIndex = "c" + index;
        this.mPredClass = classNames[cIndex];
    };
    Classifier.prototype.getTime = function () {
